...
;(function() {
  ...
  const CORE_ERROR_TEXT = 'Unsupported core-js use. Try https://npms.io/search?q=ponyfill.', FUNC_ERROR_TEXT = 'Expected a function', INVALID_TEMPL_VAR_ERROR_TEXT = 'Invalid `variable` option passed into `_.template`';
  ...
  const COMPARE_PARTIAL_FLAG = 1, COMPARE_UNORDERED_FLAG = 2;

  /** Used to compose bitmasks for function metadata. */
  const WRAP_BIND_FLAG = 1, WRAP_BIND_KEY_FLAG = 2, WRAP_CURRY_BOUND_FLAG = 4, WRAP_CURRY_FLAG = 8, WRAP_CURRY_RIGHT_FLAG = 16, WRAP_PARTIAL_FLAG = 32, WRAP_PARTIAL_RIGHT_FLAG = 64, WRAP_ARY_FLAG = 128, WRAP_REARG_FLAG = 256, WRAP_FLIP_FLAG = 512;
  ...
  const LAZY_FILTER_FLAG = 1, LAZY_MAP_FLAG = 2, LAZY_WHILE_FLAG = 3;

  /** Used as references for various `Number` constants. */
  const INFINITY = 1 / 0, MAX_SAFE_INTEGER = 9007199254740991, MAX_INTEGER = 1.7976931348623157e+308, NAN = 0 / 0;

  /** Used as references for the maximum length and index of an array. */
  const MAX_ARRAY_LENGTH = 4294967295, MAX_ARRAY_INDEX = MAX_ARRAY_LENGTH - 1, HALF_MAX_ARRAY_LENGTH = MAX_ARRAY_LENGTH >>> 1;
  ...
  const reEmptyStringLeading = /\b__p \+= '';/g, reEmptyStringMiddle = /\b(__p \+=) '' \+/g, reEmptyStringTrailing = /(__e\(.*?\)|\b__t\)) \+\n'';/g;

  /** Used to match HTML entities and HTML characters. */
  const reEscapedHtml = /&(?:amp|lt|gt|quot|#39);/g, reUnescapedHtml = /[&<>"']/g, reHasEscapedHtml = RegExp(reEscapedHtml.source), reHasUnescapedHtml = RegExp(reUnescapedHtml.source);

  /** Used to match template delimiters. */
  const reEscape = /<%-([\s\S]+?)%>/g, reEvaluate = /<%([\s\S]+?)%>/g, reInterpolate = /<%=([\s\S]+?)%>/g;

  /** Used to match property names within property paths. */
  const reIsDeepProp = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/, reIsPlainProp = /^\w*$/, rePropName = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g;
  ...
  const freeParseFloat = parseFloat, freeParseInt = parseInt;
  ...
  const nodeIsArrayBuffer = nodeUtil && nodeUtil.isArrayBuffer, nodeIsDate = nodeUtil && nodeUtil.isDate, nodeIsMap = nodeUtil && nodeUtil.isMap, nodeIsRegExp = nodeUtil && nodeUtil.isRegExp, nodeIsSet = nodeUtil && nodeUtil.isSet, nodeIsTypedArray = nodeUtil && nodeUtil.isTypedArray;
  ...
  function countHolders(array, placeholder) {
    let length = array.length, result = 0;
  ...
  const runInContext = (function runInContext(context) {
    ...
    const Array = context.Array, Date = context.Date, Error = context.Error, Function = context.Function, Math = context.Math, Object = context.Object, RegExp = context.RegExp, String = context.String, TypeError = context.TypeError;
    ...
    const Buffer = moduleExports ? context.Buffer : undefined, Symbol = context.Symbol, Uint8Array = context.Uint8Array, allocUnsafe = Buffer ? Buffer.allocUnsafe : undefined, getPrototype = overArg(Object.getPrototypeOf, Object), objectCreate = Object.create, propertyIsEnumerable = objectProto.propertyIsEnumerable, splice = arrayProto.splice, spreadableSymbol = Symbol ? Symbol.isConcatSpreadable : undefined, symIterator = Symbol ? Symbol.iterator : undefined, symToStringTag = Symbol ? Symbol.toStringTag : undefined;
    ...
    /** Mocked built-ins. */
    const ctxClearTimeout = context.clearTimeout !== root.clearTimeout && context.clearTimeout, ctxNow = Date && Date.now !== root.Date.now && Date.now, ctxSetTimeout = context.setTimeout !== root.setTimeout && context.setTimeout;

    /* Built-in method references for those with the same name as other `lodash` methods. */
    const nativeCeil = Math.ceil, nativeFloor = Math.floor, nativeGetSymbols = Object.getOwnPropertySymbols, nativeIsBuffer = Buffer ? Buffer.isBuffer : undefined, nativeIsFinite = context.isFinite, nativeJoin = arrayProto.join, nativeKeys = overArg(Object.keys, Object), nativeMax = Math.max, nativeMin = Math.min, nativeNow = Date.now, nativeParseInt = context.parseInt, nativeRandom = Math.random, nativeReverse = arrayProto.reverse;

    /* Built-in method references that are verified to be native. */
    const DataView = getNative(context, 'DataView'), Map = getNative(context, 'Map'), Promise = getNative(context, 'Promise'), Set = getNative(context, 'Set'), WeakMap = getNative(context, 'WeakMap'), nativeCreate = getNative(Object, 'create');
    ...
    const symbolProto = Symbol ? Symbol.prototype : undefined, symbolValueOf = symbolProto ? symbolProto.valueOf : undefined, symbolToString = symbolProto ? symbolProto.toString : undefined;
    ...
    function lazyValue() {
      ...
      while (length-- && resIndex < takeCount) {
        ...
        let iterIndex = -1, value = array[index];

        while (++iterIndex < iterLength) {
          const data = iteratees[iterIndex], iteratee = data.iteratee, type = data.type, computed = iteratee(value);
    ...
    function baseIsEqualDeep(object, other, bitmask, customizer, equalFunc, stack) {
      ...
      if (!(bitmask & COMPARE_PARTIAL_FLAG)) {
        const objIsWrapped = objIsObj && hasOwnProperty.call(object, '__wrapped__'), othIsWrapped = othIsObj && hasOwnProperty.call(other, '__wrapped__');

        if (objIsWrapped || othIsWrapped) {
          const objUnwrapped = objIsWrapped ? object.value() : object, othUnwrapped = othIsWrapped ? other.value() : other;
    ...
    function baseIsMatch(object, source, matchData, customizer) {
      ...
      while (++index < length) {
        ...
        const key = data[0], objValue = object[key], srcValue = data[1];
    ...
    function baseKeysIn(object) {
      ...
      const isProto = isPrototype(object), result = [];
    ...
    function baseMergeDeep(object, source, key, srcIndex, mergeFunc, customizer, stack) {
      const objValue = safeGet(object, key), srcValue = safeGet(source, key), stacked = stack.get(srcValue);
      ...
      if (isCommon) {
        const isArr = isArray(srcValue), isBuff = !isArr && isBuffer(srcValue), isTyped = !isArr && !isBuff && isTypedArray(srcValue);
    ...
    function basePickBy(object, paths, predicate) {
      ...
      while (++index < length) {
        const path = paths[index], value = baseGet(object, path);
    ...
    function baseSortedIndex(array, value, retHighest) {
      ...
      if (typeof value == 'number' && value === value && high <= HALF_MAX_ARRAY_LENGTH) {
        while (low < high) {
          const mid = (low + high) >>> 1, computed = array[mid];
    ...
    function baseSortedIndexBy(array, value, iteratee, retHighest) {
      ...
      const valIsNaN = value !== value, valIsNull = value === null, valIsSymbol = isSymbol(value), valIsUndefined = value === undefined;

      while (low < high) {
        const mid = nativeFloor((low + high) / 2), computed = iteratee(array[mid]), othIsDefined = computed !== undefined, othIsNull = computed === null, othIsReflexive = computed === computed, othIsSymbol = isSymbol(computed);
    ...
    function cloneBuffer(buffer, isDeep) {
      ...
      const length = buffer.length, result = allocUnsafe ? allocUnsafe(length) : new buffer.constructor(length);
    ...
    function compareAscending(value, other) {
      if (value !== other) {
        const valIsDefined = value !== undefined, valIsNull = value === null, valIsReflexive = value === value, valIsSymbol = isSymbol(value);

        const othIsDefined = other !== undefined, othIsNull = other === null, othIsReflexive = other === other, othIsSymbol = isSymbol(other);
    ...
    function createAggregator(setter, initializer) {
      return function(collection, iteratee) {
        const func = isArray(collection) ? arrayAggregator : baseAggregator, accumulator = initializer ? initializer() : {};
    ...
    function createBind(func, bitmask, thisArg) {
      const isBind = bitmask & WRAP_BIND_FLAG, Ctor = createCtor(func);
    ...
    function createCtor(Ctor) {
      return function() {
        ...
        const thisBinding = baseCreate(Ctor.prototype), result = Ctor.apply(thisBinding, args);
    ...
    function createFlow(fromRight) {
      return flatRest(function(funcs) {
        ...
        while (++index < length) {
          ...
          const funcName = getFuncName(func), data = funcName == 'wrapper' ? getData(func) : undefined;
        ...
        return function() {
          const args = arguments, value = args[0];
    ...
    function createHybrid(func, bitmask, thisArg, partials, holders, partialsRight, holdersRight, argPos, ary, arity) {
      const isAry = bitmask & WRAP_ARY_FLAG, isBind = bitmask & WRAP_BIND_FLAG, isBindKey = bitmask & WRAP_BIND_KEY_FLAG, isCurried = bitmask & (WRAP_CURRY_FLAG | WRAP_CURRY_RIGHT_FLAG), isFlip = bitmask & WRAP_FLIP_FLAG, Ctor = isBindKey ? undefined : createCtor(func);

      function wrapper() {
        let length = arguments.length, args = Array(length), index = length;
    ...
    function createRecurry(func, bitmask, wrapFunc, placeholder, thisArg, partials, holders, argPos, ary, arity) {
      const isCurry = bitmask & WRAP_CURRY_FLAG, newHolders = isCurry ? holders : undefined, newHoldersRight = isCurry ? undefined : holders, newPartials = isCurry ? partials : undefined, newPartialsRight = isCurry ? undefined : partials;
    ...
    function equalArrays(array, other, bitmask, customizer, equalFunc, stack) {
      const isPartial = bitmask & COMPARE_PARTIAL_FLAG, arrLength = array.length, othLength = other.length;
    ...
    function equalObjects(object, other, bitmask, customizer, equalFunc, stack) {
      const isPartial = bitmask & COMPARE_PARTIAL_FLAG, objProps = getAllKeys(object), objLength = objProps.length, othProps = getAllKeys(other), othLength = othProps.length;
      ...
      if (result && !skipCtor) {
        const objCtor = object.constructor, othCtor = other.constructor;
    ...
    function getFuncName(func) {
      ...
      while (length--) {
        const data = array[length], otherFunc = data.func;
    ...
    function getMatchData(object) {
      ...
      while (length--) {
        const key = result[length], value = object[key];
    ...
    if ((DataView && getTag(new DataView(new ArrayBuffer(1))) != dataViewTag) ||
      ...
      getTag = function(value) {
        const result = baseGetTag(value), Ctor = result == objectTag ? value.constructor : undefined, ctorString = Ctor ? toSource(Ctor) : '';
    ...
    function getView(start, end, transforms) {
      ...
      while (++index < length) {
        const data = transforms[index], size = data.size;
    ...
    function initCloneArray(array) {
      const length = array.length, result = new array.constructor(length);
    ...
    function isLaziable(func) {
      const funcName = getFuncName(func), other = lodash[funcName];
    ...
    function wrapperNext() {
      ...
      const done = this.__index__ >= this.__values__.length, value = done ? undefined : this.__values__[this.__index__++];
    ...
    function reduce(collection, iteratee, accumulator) {
      const func = isArray(collection) ? arrayReduce : baseReduce, initAccum = arguments.length < 3;
    ...
    function reduceRight(collection, iteratee, accumulator) {
      const func = isArray(collection) ? arrayReduceRight : baseReduce, initAccum = arguments.length < 3;
    ...
    function memoize(func, resolver) {
      ...
      const memoized = function() {
        const args = arguments, key = resolver ? resolver.apply(this, args) : args[0], cache = memoized.cache;
    ...
    function spread(func, start) {
      ...
      return baseRest(function(args) {
        const array = args[start], otherArgs = castSlice(args, 0, start);
    ...
    function toArray(value) {
      ...
      const tag = getTag(value), func = tag == mapTag ? mapToArray : (tag == setTag ? setToArray : values);
    ...
    function transform(object, iteratee, accumulator) {
      const isArr = isArray(object), isArrLike = isArr || isBuffer(object) || isTypedArray(object);
     ...
     */
    function replace() {
      const args = arguments, string = toString(args[0]);
    ...
    function template(string, options, guard) {
      ...
      const imports = assignInWith({}, options.imports, settings.imports, customDefaultsAssignIn), importsKeys = keys(imports), importsValues = baseValues(imports, importsKeys);
    ...
    function trim(string, chars, guard) {
      ...
      const strSymbols = stringToArray(string), chrSymbols = stringToArray(chars), start = charsStartIndex(strSymbols, chrSymbols), end = charsEndIndex(strSymbols, chrSymbols) + 1;
    ...
    function trimEnd(string, chars, guard) {
      ...
      const strSymbols = stringToArray(string), end = charsEndIndex(strSymbols, stringToArray(chars)) + 1;
    ...
    function trimStart(string, chars, guard) {
      ...
      const strSymbols = stringToArray(string), start = charsStartIndex(strSymbols, stringToArray(chars));
    ...
    function truncate(string, options) {
      let length = DEFAULT_TRUNC_LENGTH, omission = DEFAULT_TRUNC_OMISSION;
     ...
     */
    function cond(pairs) {
      const length = pairs == null ? 0 : pairs.length, toIteratee = getIteratee();
    ...
    function mixin(object, source, options) {
      ...
      const chain = !(isObject(options) && 'chain' in options) || !!options.chain, isFunc = isFunction(object);
      ...
      arrayEach(methodNames, function(methodName) {
        ...
        if (isFunc) {
          object.prototype[methodName] = function() {
            ...
            if (chain || chainAll) {
              const result = object(this.__wrapped__), actions = result.__actions__ = copyArray(this.__actions__);
    ...
    arrayEach(['filter', 'map', 'takeWhile'], function(methodName, index) {
      const type = index + 1, isFilter = type == LAZY_FILTER_FLAG || type == LAZY_WHILE_FLAG;
    ...
    baseForOwn(LazyWrapper.prototype, function(func, methodName) {
      const checkIteratee = /^(?:filter|find|map|reject)|While$/.test(methodName), isTaker = /^(?:head|last)$/.test(methodName), lodashFunc = lodash[isTaker ? ('take' + (methodName == 'last' ? 'Right' : '')) : methodName], retUnwrapped = isTaker || /^find/.test(methodName);
    ...
    arrayEach(['pop', 'push', 'shift', 'sort', 'splice', 'unshift'], function(methodName) {
      const func = arrayProto[methodName], chainName = /^(?:push|sort|unshift)$/.test(methodName) ? 'tap' : 'thru', retUnwrapped = /^(?:pop|shift)$/.test(methodName);
