...
;((() => {
  ...
  function baseIsEqualDeep(object, other, bitmask, customizer, equalFunc, stack) {
    ...
    stack || (stack = []);
    const objStack = find(stack, entry => {
      return entry[0] == object;
    });
    const othStack = find(stack, entry => {
  ...
  function baseWrapperValue(value, actions) {
    const result = value;
    return reduce(actions, (result, {func, thisArg, args}) => {
  ...
  function sortBy(collection, iteratee) {
    ...
    return baseMap(baseMap(collection, (value, key, collection) => {
   ...
   */
  const defer = baseRest((func, args) => {
   ...
   */
  const delay = baseRest((func, wait, args) => {
   ...
   * // => false
   */
  var isArguments = baseIsArguments(((...args) => { return args; })()) ? baseIsArguments : value => {
   ...
   * // => { 'a': 1, 'c': 3 }
   */
  const pick = flatRest((object, paths) => {
      ...
      if (isFunc) {
        object.prototype[methodName] = function(...args) {
          ...
          if (chain || chainAll) {
            ...
            actions.push({ 'func': func, 'args': args, 'thisArg': object });
            result.__chain__ = chainAll;
            return result;
          }
          return func.apply(object, arrayPush([this.value()], args));
        };
      }
    });
