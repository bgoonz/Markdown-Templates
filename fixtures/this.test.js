...
test("files in ignored list are not searchable", () => {
  ...
  expect(Object.keys(index).find((e) => e.indexOf("/devnull/") === 0)).toBe(
    undefined
  );
});
...
test("empty search index returns 400", async () => {
  ...
  const { statusCode } = await searchHandler({
...
test("non-empty search index returns results", async () => {
  const { statusCode, body } = await searchHandler({
    queryStringParameters: { search: "developers" },
  });
...
test("limit qury param should limit results len", async () => {
  const { statusCode, body } = await searchHandler({
    queryStringParameters: { search: "developers", limit: "2" },
  });
  expect(statusCode).toBe(200);
  const results = JSON.parse(body);
...
test("NaN should not break search", async () => {
  const { statusCode, body } = await searchHandler({
    queryStringParameters: { search: "developers", limit: "nan" },
  });
  expect(statusCode).toBe(200);
  const results = JSON.parse(body);
